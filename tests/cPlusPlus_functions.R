...
df.bs <- blackScholes(S=spot, K=strike, T=t2m, r=rf, sigma=sigma, b=costOfCarry, option_type = "Call", nominal=nominal, position_str = "Long", option_cost=premium) ## Black-Scholes
df.bt <- binomialTree(S=spot, K=strike, T=t2m, r=rf, u=upFactor, d=downFactor, p=prob, steps=steps, option_type = "Put", nominal=nominal, position_str = "Long", option_cost=premium) ## Binomial Tree
...
df.cds <- creditDefaultSwap(credit_spread=creditSpread, default_probability_start=prob, recovery_rate=recovery, period_length=t2m, discount_rate=rf, position_str="Long", nominal=nominal)
